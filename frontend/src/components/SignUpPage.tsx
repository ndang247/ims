...
const SignupPage = () => {
  ...
  return (
    <Card title="Sign Up For DC Warehouse" style={{ width: 400, margin: "auto", marginTop: 50 }}>
      <Form
        form={form}
        name="signup"
      ...
      >
        ...
        <Form.Item name="warehouse" rules={[{ required: true, message: 'Please select a warehouse!' }]}>
          <Select placeholder="Select a warehouse">
            {warehouses.map((warehouse) => (
              <Option key={warehouse._id} value={warehouse._id}>{warehouse.name} - {warehouse.address}</Option>
            ))}
          </Select>
        </Form.Item>
